<?php
	class donhang
	{
		function danh_sach()
		{
			$sql = "select * from  dondathang where dh_trangthaithanhtoan = 1";
		...
		function set_edit($ma)
		{
			...
			if($status[4] == 2)
				$sql = "update dondathang set dh_trangthaithanhtoan =1 where dh_ma=$ma";
			else if($status[4] == 1)
				$sql = "update dondathang set dh_trangthaithanhtoan =2 where dh_ma=$ma";
			mysql_query($sql);
		}
